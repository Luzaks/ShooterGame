export class CommandsScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.add.text(20, 20, 'COMMANDS AND MOVEMENTS', {fonFamily: 'Calibri', fontSize: '30px',}).setDepth(1);

        this.hover = this.sound.add('buttonSound');
        this.pressed = this.sound.add( 'buttonPressed');

        this.add.text(330, 160, 'Moving around', {fonFamily: 'Calibri', fontSize: '20px',}).setDepth(1);

        this.add.image(297, 200, 'movesKey').setOrigin(0, 0).setDepth(1);

        this.add.text(350, 350, 'Shooting', {fonFamily: 'Calibri', fontSize: '20px',}).setDepth(1);
        this.add.image(312, 390, 'spaceKey').setOrigin(0, 0).setDepth(1);


        let playButtonCommand = this.add.image(495, 460, 'playButtonIntro').setOrigin(0, 0);
        let hoverPlayButtonCommand = this.add.image(495, 460, 'playButtonIntroHover').setOrigin(0, 0);
        hoverPlayButtonCommand.setVisible(false);

        let menuButtonCommand = this.add.image(95, 460, 'menuIntroButton').setOrigin(0, 0);
        let hoverMenuButtonCommand = this.add.image(95, 460, 'menuIntroButtonHover').setOrigin(0, 0);
